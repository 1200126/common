...
public class DateUtil {
     ...
     * 编号格式
     */
    public static String FORMAT_DATE = "yyyyMMddHHmmssSSS";
     ...
     * @return
     */
    public static String getAddFormatDate() {
        SimpleDateFormat dateFormat = new SimpleDateFormat(FORMAT_YMD);
    ...
    public static Date subMonth(Date date, int n) {
        ...
        calendar.add(Calendar.MONTH, -n);////当前时间前去一个月，即一个月前的时间
    ...
    /**
     * 获取时间格式: yyyy-MM-dd HH:mm
     *
     * @return
     */
    public static String getTimeHM() {
        SimpleDateFormat df = new SimpleDateFormat(FORMAT_YMDHM);
        Calendar calendar = Calendar.getInstance();
        return df.format(calendar.getTime());
    }
    ...
    /**
     * 方法名: checkIsLastTime</br>
     * 详述: 判断时间时间是否比阅读时间要靠后</br>
     * 开发人员：huangzy</br>
     * 创建时间：2016年4月22日</br>
     *
     * @param start
     ...
     * @return true 时间时间靠后
     */
    public static boolean checkTime(String start, String end,String format) {
        boolean isLast = true;
        try {
            java.util.Date startData = (new SimpleDateFormat(format)).parse(start);
            java.util.Date endData = (new SimpleDateFormat(format)).parse(end);
            isLast = (endData.getTime() - startData.getTime()) > 0;
        } catch (Throwable e) {
